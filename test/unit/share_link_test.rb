...
class ShareLinkTest < ActiveSupport::TestCase
  test 'emails is not empty' do
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => '') }
  end

  test 'link expires at is not empty' do
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :link_expires_at => '') }
  end

  test 'emails is not longer than 256 characters' do
    share_link = Factory(:share_link)
  ...
  test 'the format of emails is valid' do
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, @.com') }
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, @test.com') }
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, test@.com, another-email@domain.com') }
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, test@test.') }
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, test@$%^.com') }
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, test@test.c') }
    assert_raise(ActiveRecord::RecordInvalid) { Factory(:share_link, :emails => 'mail@domain.com, test@test.$$$') }
  end

  test 'a link token gets generated' do
    share_link1 = Factory(:share_link)
    assert !share_link1.link_token.blank?

    share_link2 = ShareLink.new
    share_link2.user_file = Factory(:user_file)
  ...
  test 'active share links' do
    ...
    Factory(:share_link, :link_expires_at => next_week)
    Factory(:share_link, :link_expires_at => in_three_weeks)
    Factory(:share_link, :link_expires_at => last_week)
  ...
  test 'the correct file is returned for a given link token' do
    ...
    share_link2 = Factory(:share_link, :link_expires_at => 1.week.ago.end_of_day)
